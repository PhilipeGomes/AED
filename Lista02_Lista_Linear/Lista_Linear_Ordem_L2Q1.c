...
void inserir(NoListSeq *lista,int elemento){
    int i,j,status = 0;
    NoListSeq l = *lista;
    if (lista->ultimo < lista->maximo){
        ...
        printf("Lista cheia, aumentando para 2*M e inserindo valor\n");
        int vetor_aux[lista->maximo*2];
        for(i=0;i<lista->maximo;i++){
            vetor_aux[i] = lista->dados[i];
        }

        lista->maximo = lista->maximo*2;
        vetor_aux[i] = elemento;
        for(j=i+1;j<lista->maximo;j++){
            vetor_aux[j] = NULL;
        }

        for(i=0;i<lista->maximo;i++){
            lista->dados[i] = vetor_aux[i];
        }
